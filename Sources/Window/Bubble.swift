...
final class Bubble: UIView {
    ...
    static var originalPosition: CGPoint {
        if HeliosTraceSettings.shared.bubbleFrameX != 0 && HeliosTraceSettings.shared.bubbleFrameY != 0 {
            return CGPoint(x: CGFloat(HeliosTraceSettings.shared.bubbleFrameX), y: CGFloat(HeliosTraceSettings.shared.bubbleFrameY))
        }
        
        var h = 0
        
        if UIApplication.shared.safeAreaTop > 24.0 {
            h = 16
        }
    ...
    fileprivate func initLayer() {
        ...
        if let numberLabel = numberLabel {
            numberLabel.text = String(self.networkNumber)
        ...
        let longTap = UILongPressGestureRecognizer(target: self, action: #selector(Bubble.longTap))
        self.addGestureRecognizer(longTap)
    }
    
    func changeSideDisplay() {
        UIView.animate(withDuration: 0.5, delay: 0.1, usingSpringWithDamping: 0.5,
                       initialSpringVelocity: 5, options: .curveEaseInOut, animations: {}, completion: nil)
    }
    ...
    override init(frame: CGRect) {
        ...
        // notification
        NotificationCenter.default.addObserver(forName: .reloadHttpHeliosTrace, object: nil, queue: OperationQueue.main) { [weak self] notification in
            self?.reloadHttp_notification(notification)
        }
        
        NotificationCenter.default.addObserver(forName: NSNotification.Name(rawValue: "deleteAllLogs_HeliosTrace"), object: nil, queue: OperationQueue.main) { [weak self] _ in
    ...
    @objc func reloadHttp_notification(_ notification: Notification) {
        ...
        self.numberLabel?.text = String(self.networkNumber)
        ...
        if self.networkNumber >= 0, self.networkNumber < 10 {
            self.numberLabel?.font = UIFont.boldSystemFont(ofSize: 11)
        } else if self.networkNumber >= 10, self.networkNumber < 100 {
            self.numberLabel?.font = UIFont.boldSystemFont(ofSize: 11)
        } else if self.networkNumber >= 100, self.networkNumber < 1000 {
            self.numberLabel?.font = UIFont.boldSystemFont(ofSize: 9)
        } else if self.networkNumber >= 1000, self.networkNumber < 10000 {
    ...
    @objc func longTap() {
        ...
        NotificationCenter.default.post(name: NSNotification.Name("deleteAllLogs_HeliosTrace"), object: nil, userInfo: nil)
    }
    ...
    @objc func panDidFire(panner: UIPanGestureRecognizer) {
        ...
        if panner.state == .ended || panner.state == .cancelled {
            ...
            frameInset = UIApplication.shared.safeAreaInsets ?? .zero
            
            let location = panner.location(in: self.superview)
            let velocity = panner.velocity(in: self.superview)
            
            var finalX = Double(self.width/8*4.25)
            var finalY = Double(location.y)
            ...
            //
            HeliosTraceSettings.shared.bubbleFrameX = Float(finalX)
            HeliosTraceSettings.shared.bubbleFrameY = Float(finalY)
